...
def calculate_edge_dist(sampleLst, r1, r2, a, b):
    ...
    #Oval Shape 
    x3 = - sampleLst['x3']             # HAD TO FIX THESE TO BE CONSISTENT WITH DEFINITON IN FREECAD FILE
    y3 = - sampleLst['y3']
    ...
    x_oval_max = max( abs(x_oval_1), abs(x_oval_2), abs(x_oval_3), abs(x_oval_4))
    ...
    y_oval_max = max( abs(y_oval_1), abs(y_oval_2), abs(y_oval_3), abs(y_oval_4))

    d_oval = 30 - max( x_oval_max, y_oval_max)
    ...
    return (d_circle1 > clearance) & (d_circle2>clearance) & (d_oval>clearance)
